...
public class Store implements Serializable {
    ...
    private int isVisitedApi;

    @SerializedName("is_order_taken")
    private int isOrderTakenApi;

    // --- Local UI State ---
    private boolean isVisited = false;
    private boolean isOrderTaken = false;
//    private int isVisited = 1;
//    private int isOrderTaken = 1;
    private double orderValue = 0.0;
...
public String getAddress() {
    ...
    public double getLat() {
        if (latLong != null && latLong.contains(",")) {
            try { return Double.parseDouble(latLong.split(",")[0].trim()); }
    ...
    public double getLng() {
        if (latLong != null && latLong.contains(",")) {
            try { return Double.parseDouble(latLong.split(",")[1].trim()); }
            catch (Exception ignored) {}
        }
        return 0.0;
    }
    ...
    // --- API Status Mapping ---
    public boolean isVisited() {
        return isVisitedApi == 1 || isVisited;
    }

    public boolean isOrderTaken() {
        return isOrderTakenApi == 1 || isOrderTaken;
    }

    // --- Optional Local Setters ---
    public void setVisited(boolean visited) {
        this.isVisited = visited;
    }

    public void setOrderTaken(boolean orderTaken) {
        this.isOrderTaken = orderTaken;
    }
    public int getIsVisitedApi() {
        return isVisitedApi;
    }
    public int getIsOrderTakenApi() {
        return isOrderTakenApi;
    }
