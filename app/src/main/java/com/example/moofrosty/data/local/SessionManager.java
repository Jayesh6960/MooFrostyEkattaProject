...
////    }
////
////    public void g(String token, String fullJsonString) {
...
public class SessionManager {
    ...
    private static final String KEY_IS_PRESENT = "is_present";
    private static final String KEY_IS_PRESENT_OUT = "is_present_out";
    private static final String KEY_ORDER_TAKEN = "order_taken";
    ...
    public LoginResponse.User getCurrentUser() {
        String userJson = pref.getString(KEY_USER_JSON, "");
        if (userJson == null || userJson.isEmpty() || userJson.equals("{}")) {
            return null;
        }
        try {
            LoginResponse loginResponse = new Gson().fromJson(userJson, LoginResponse.class);
    ...
    public String getUserFullName() {
        ...
        try {
            ...
            if (userObj != null) {
                ...
                fullName = middle.isEmpty() ? first + " " + last : first + " " + middle + " " + last;
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    public void saveBeat(int beatId, String beatName) {
        editor.putInt(KEY_BEAT_ID, beatId);
        editor.putString(KEY_BEAT_NAME, beatName);
        editor.apply();
    }

    // ------------------- ATTENDANCE -------------------
    public boolean isAttendanceMarked() {
        return pref.getBoolean(KEY_IS_PRESENT, false);
    }
    ...
    public boolean isLocationAndPermissionsEnabled() {
        ...
        boolean backgroundLocationGranted = true;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            backgroundLocationGranted = ContextCompat.checkSelfPermission(context,
                    android.Manifest.permission.ACCESS_BACKGROUND_LOCATION)
                    == PackageManager.PERMISSION_GRANTED;
        }
        ...
        boolean allPermissionsGranted = fineLocation && backgroundLocationGranted && cameraGranted && contactsGranted;
