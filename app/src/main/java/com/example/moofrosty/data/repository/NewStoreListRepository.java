...
//        });
//    }
  //  above code storelist below page

    public void fetchStoresByDate(String token, String date, String page,
                                  MutableLiveData<Resource<List<StoreListResponse.StoreModel>>> liveData) {

        liveData.postValue(Resource.loading(null));

        apiService.getStoreList("Bearer " + token, date, page)
                .enqueue(new Callback<StoreListResponse>() {

                    @Override
                    public void onResponse(Call<StoreListResponse> call,
                                           Response<StoreListResponse> response) {
