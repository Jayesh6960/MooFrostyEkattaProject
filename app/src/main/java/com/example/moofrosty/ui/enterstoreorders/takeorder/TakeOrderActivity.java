...
//            boolean intentValue = getIntent().getBooleanExtra("IS_ORDER_TAKEN", false);
//            boolean sessionValue = sessionManager.isOrderTaken();

//            isOrderTaken = intentValue || sessionValue;
//
//            Log.d("ORDER_FLOW", "Intent: " + intentValue +
...
//                });
        ...
        cartViewModel.getCartTotals().observe(this, totals -> {
            ...
            if (totals.uniqueItemCount == 0) {
                ...
                floatingCartBar.setVisibility(GONE);
...
//    // ------------------------------
//
//    boolean isOrderTaken = sessionManager.isOrderTaken();
//
//    String[] reasons;
//
//    if (isOrderTaken) {
...
//            logoutUser();
//            bottomSheetDialog.dismiss();
//            Toast.makeText(this, "Checkout Sucessfully done ", Toast.LENGTH_SHORT).show();
    ////            Toast.makeText(this, "Outlet data Missing ", Toast.LENGTH_SHORT).show();
//        }
//    });
//
...
//    bottomSheetDialog.show();
//}
//need to still work over it  date
    @Override
    protected void onNewIntent(Intent intent) {
        super.onNewIntent(intent);
        // This is critical! It updates the activity's intent with the new one from CartActivity
        setIntent(intent);

        // Refresh currentStore from the new intent
        if (intent.getSerializableExtra("STORE_DATA") != null) {
            currentStore = (Store) intent.getSerializableExtra("STORE_DATA");
            Log.d("STORE_DATA", "onNewIntent: Received " + currentStore.getStoreName());
        }
    }
    ...
    private void showCheckOutDialog() {
        ...
        BottomSheetDialog bottomSheetDialog = new BottomSheetDialog(this);
        View sheetView = LayoutInflater.from(this).inflate(R.layout.bottom_layout_material, null);

        bottomSheetDialog.setContentView(sheetView);

        TextInputEditText tvStore = sheetView.findViewById(R.id.tvStore);
        TextInputLayout outlet = sheetView.findViewById(R.id.outletname);
        TextInputLayout tilReason = sheetView.findViewById(R.id.tilReason);
        AutoCompleteTextView spReason = sheetView.findViewById(R.id.spReason);
        MaterialButton btnConfirm = sheetView.findViewById(R.id.btnConfirm);
        ImageView ivClose = sheetView.findViewById(R.id.ivClose);

        boolean isOrderPlaced = sessionManager.isOrderTaken();
        if (currentStore != null) {
            tvStore.setText(currentStore.getStoreName());
        }

        int currentShopId = (currentStore != null) ? currentStore.getShopId() : sessionManager.getShopId();
        int savedShopId = sessionManager.getShopId();
        boolean isOrderTakenForThisStore = sessionManager.isOrderTaken() && (currentShopId == savedShopId);

//        String storeName = (currentStore != null) ? currentStore.getStoreName() : "Outlet";
...
//            outlet.setVisibility(View.GONE);
            tilReason.setEndIconMode(TextInputLayout.END_ICON_NONE);
            ...
            spReason.setFocusable(false);
            spReason.setClickable(false);
//            spReason.setOnTouchListener((v, event) -> true);

        } else {
            reasons = new String[]{"Stock Available", "Outlet is Closed", "Owner is not available"};
            ...
            spReason.setText("", false);
            spReason.setEnabled(true);
            spReason.setFocusable(true);
            spReason.setClickable(true);
        }
...
////Temporatu solution for tghe log out/
//                if (currentStore == null) {
////                    String shopId = String.valueOf(sessionManager.getShopId());
////                    takeOrderViewModel.performCheckOut(
...
////                    );
//                    logoutUser();
//                    // Call API using saved shopId
//                    takeOrderViewModel.getCheckoutStatus();
//                    Toast.makeText(this, "Checkout Sucessfully done ", Toast.LENGTH_SHORT).show();
//                    finish();
//
//                }
...
//                );
            ...
            String token = sessionManager.getToken();
            int shopId = (currentStore != null) ? currentStore.getShopId() : sessionManager.getShopId();

            if (shopId != 0) {
                // Capture and save Out Time
//            String outTime = getCurrentDateTime();
//            getSharedPreferences("store_data", MODE_PRIVATE).edit()
//                    .putString("store_out_time", outTime).apply();

                // Perform API Checkout
                takeOrderViewModel.performCheckOut(token, shopId, selectedReason);
                bottomSheetDialog.dismiss();
            } else {
                // Fallback: If everything fails, logout to prevent stuck state
                Toast.makeText(this, "Outlet Data Missing", Toast.LENGTH_SHORT).show();
                // logoutUser();
                bottomSheetDialog.dismiss();
            }
...
//    }
    ...
    private void performExitLogic() {
        ...
        sessionManager.setOrderTaken(false);
//            sessionManager.clearShopId();
        sessionManager.setLastCheckoutReason("");
        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);
        startActivity(intent);
        finish();
    }
    ...
    private final BottomNavigationView.OnItemSelectedListener navListener =
            item -> {
                ...
                if (isMenuFragment) {
                    ...
                    relativescreen.setVisibility(GONE);
                }
